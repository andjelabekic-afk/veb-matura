...
Array.from(document.querySelectorAll(".saznaj-vise")).forEach(link => {
    link.addEventListener("click", e => {
        e.preventDefault();
        window.open(link.href, "Instrumenti", "width=500,height=300,left=100,top=100,menubar=no,toolbar=no");
    });
});

Array.from(document.querySelectorAll(".zvucnik img")).forEach(zv => {
